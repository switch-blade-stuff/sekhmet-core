...
namespace sek
{
	...
	private:
		...
		[[nodiscard]] bool read_frame_header(frame_header &header)
		{
			...
			if (read_checked(&frame, sizeof(frame)) && frame.magic == SEK_BSWAP_LE_32(skip_magic) &&
				frame.size == SEK_BSWAP_LE_32(sizeof(frame_header))) [[likely]]
			{
				header.comp_size = SEK_BSWAP_LE_32(frame.header.comp_size);
				header.src_size = SEK_BSWAP_LE_32(frame.header.src_size);
				return true;
			}
			return false;
		}
