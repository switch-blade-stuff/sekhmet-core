...
namespace sek
{
	...
	public:
		...
		constexpr iterator emplace(const_iterator where, Args &&...args)
		{
			...
			const auto result = make_space(where, 1);
			try
			{
			...
			catch (...)
			{
				...
				if (back_emplace) m_size.value(m_size.size() - 1);
				throw;
			}
			return result;
		}
		...
		{
			const auto back_emplace = where == end();
			const auto result = make_space(where, n);
			size_type i = 0;
			try
			{
				while (i < n) std::construct_at(result.get() + (i++), value);
			}
			catch (...)
			{
				/* Follow the strong exception guarantee of `std::vector` and rollback the size. */
				if (back_emplace && i == n) m_size.value(m_size.size() - 1);
				throw;
			}
			return result;
		}
		...
		constexpr iterator insert(const_iterator where, I first, S last)
		{
			...
			try
			{
				for (auto dst = result.get(); first < last; ++dst) std::construct_at(dst, *(first++));
			}
			catch (...)
			{
				/* Follow the strong exception guarantee of `std::vector` and rollback the size. */
		...
		friend constexpr void swap(buffered_vector &a, buffered_vector &b) noexcept requires std::is_swappable_v<T> { a.swap(b); }
		// clang-format on
