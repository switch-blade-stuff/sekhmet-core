...
namespace sek
{
	namespace detail
	{
		...
		template<tuple_like T, typename A, typename... Args, std::size_t I, std::size_t... Is>
	...
	void serialize(const T &tuple, A &archive, Args &&...args) requires(!std::ranges::forward_range<T> && tuple_like<T>)
	{
	...
	template<typename T, typename A, typename... Args>
	void deserialize(T &tuple, A &archive, Args &&...args) requires(!std::ranges::forward_range<T> && tuple_like<T>)
	{
		if constexpr (std::tuple_size_v<T> != 0)
			detail::tuple_deserialize_unwrap(std::make_index_sequence<std::tuple_size_v<T>>{}, tuple, archive, std::forward<Args>(args)...);
	}

	template<pair_like T, typename A, typename... Args>
	void serialize(const T &pair, A &archive, Args &&...args) requires(!tuple_like<T>)
	{
	...
	template<pair_like T, typename A, typename... Args>
	void deserialize(T &pair, A &archive, Args &&...args) requires(!tuple_like<T>)
	{
