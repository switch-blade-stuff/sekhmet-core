...
namespace sek
{
	...
	public:
		...
		[[nodiscard]] constexpr auto find(const auto &key) noexcept requires transparent_key
		{
			return m_table.find(key);
		}
		/** @copydoc find */
		[[nodiscard]] constexpr auto find(const auto &key) const noexcept requires transparent_key
		{
			return m_table.find(key);
		}
		/** @copydoc contains
		 * @note This overload participates in overload resolution only
		 * if both key hasher and key comparator are transparent. */
		[[nodiscard]] constexpr bool contains(const auto &key) const noexcept requires transparent_key
		{
			return find(key) != end();
		}
		// clang-format on
		 ...
		 * @note This overload participates in overload resolution only
		 * if both key hasher and key comparator are transparent. */
		[[nodiscard]] constexpr mapped_type &at(const auto &key) requires transparent_key
		{
			if (auto iter = find(key); iter != end()) [[likely]]
				return iter->second;
			else
		...
		/** @copydoc at */
		[[nodiscard]] constexpr const mapped_type &at(const auto &key) const requires transparent_key
		{
			if (auto iter = find(key); iter != end()) [[likely]]
				return iter->second;
			else
